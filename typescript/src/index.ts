...
export function aplicarDescuentoRegistros(
  ...
  return productos.map((prod) => ({
    ...prod,
    precio: parseFloat((prod.precio * (1 - porcentaje / 100)).toFixed(2)),
  }));
}
...
export function contarPalabras(texto: string): Record<string, number> {
  ...
  return palabras.reduce<Record<string, number>>((acc, palabra) => {
    return { ...acc, [palabra]: (acc[palabra] || 0) + 1 };
  }, {});
}
...
export function promedioAprobados(
  ...
  return estudiantes
    ...
    .map((est) => est.nota) // array de notas aprobados
    .reduce((acc, nota, _, arr) => acc + nota / arr.length, 0); //el indice no lo uso
}
...
export function estadisticasArray(nums: number[]): {
  ...
  const vacio = nums.length === 0;

  const minimo = vacio ? 0 : Math.min(...nums);
  const maximo = vacio ? 0 : Math.max(...nums);
  const suma = nums.reduce((acc, num) => acc + num, 0);
  const promedio = vacio ? 0 : suma / nums.length;

  const ordenados = vacio ? [] : [...nums].sort((a, b) => a - b);
  const mediana = vacio
    ? 0
    : nums.length % 2 === 0
      ? (ordenados[Math.floor(nums.length / 2) - 1] +
         ordenados[Math.floor(nums.length / 2)]) / 2
      : ordenados[Math.floor(nums.length / 2)];
  return { min: minimo, max: maximo, sum: suma, promedio, mediana };
}
...
export function transformarDatos(
  ...
  return registros
    .filter((reg) => reg.activo)
    .map((reg) => ({
      nombre: reg.nombre,
      promedio:
        reg.ventas.length > 0
          ? parseFloat((reg.ventas.reduce((acc, v) => acc + v, 0) / reg.ventas.length).toFixed(2))
          : 0,
    }))
